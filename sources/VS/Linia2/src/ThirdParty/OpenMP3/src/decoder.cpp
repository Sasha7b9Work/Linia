...
bool OpenMP3::Decoder::Private::ReadSideInfo(Decoder & self, const Frame & frame)
{
	...
	UInt nch = (UInt)(mono ? 1 : 2);

	UInt sideinfo_size = (UInt)(mono ? 17 : 32);
	...
	Local::GetSideBits((UInt)(mono ? 5 : 3), ptr, idx);	//skip private bits
...
bool OpenMP3::Decoder::Private::ReadMain(Decoder & self, const Frame & frame)
{
	...
	UInt nch = (UInt)(frame.m_mode == OpenMP3::kModeMono ? 1 : 2);

	UInt sideinfo_size = (UInt)(nch == 1 ? 17 : 32);
