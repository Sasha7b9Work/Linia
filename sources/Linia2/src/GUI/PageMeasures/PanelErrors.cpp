// 2025/09/01 22:59:47 (c) Aleksandr Shevchenko e-mail : Sasha7b9@gmail.com
#include "defines.h"
...
void PanelErrors::ReInit()
{
    ...
    size.x = ThePageMeasures->GetFullSize().x;
...
void PanelErrors::OnEventButton(wxCommandEvent &event)
{
    ...
    if (collapse)
    {
        ...
        btnCollapse->SetLabel(L("Развернуть"));
        timer.Start(500);
    }
    else
    {
        SetSize({ GetSize().x, 100 });
        btnCollapse->SetLabel(L("Свернуть"));
        timer.Stop();
    }
