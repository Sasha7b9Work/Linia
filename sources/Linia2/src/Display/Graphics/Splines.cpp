...
// Реализация SplineRenderer
void SplineRenderer::DrawSpline(wxDC &dc, const std::vector<wxPoint> &points,
    double step, int boundary)
{
    ...
    {
        dc.DrawLine(splinePoints[i], splinePoints[i + 1]);
    }
}


void SplineRenderer::DrawFilledSpline(wxDC &dc, const std::vector<wxPoint> &points,
    int baselineY, double step)
{
    if (points.size() < 2) return;

    CubicSpline spline;
    if (!spline.Initialize(points)) return;

    auto splinePoints = spline.GetPoints(step);
    if (splinePoints.empty()) return;

    // Создаем точки для заливки области
    std::vector<wxPoint> fillPoints = splinePoints;

    // Добавляем точки для замыкания области с базовой линией
    if (baselineY == 0)
    {
        baselineY = points.back().y;
    }

    fillPoints.emplace_back(splinePoints.back().x, baselineY);
    fillPoints.emplace_back(splinePoints.front().x, baselineY);
    fillPoints.push_back(splinePoints.front());

    // Рисуем залитую область
    dc.DrawPolygon((int)fillPoints.size(), fillPoints.data());

    // Рисуем сам сплайн поверх заливки
...
void SplineRenderer::DrawSplineWithMarkers(wxDC &dc, const std::vector<wxPoint> &points,
    double step)
{
    // Рисуем сплайн
    DrawSpline(dc, points, step);

    // Рисуем маркеры в исходных точках
