...
void ConsoleRS232::Create()
{
    if (!self)
    {
        ...
        self->SetPosition(SET_GUI_POS_CONSOLE->GetPoint());

        self->SetClientSize(SET_GUI_SIZE_CONSOLE->GetPoint().x, SET_GUI_SIZE_CONSOLE->GetPoint().y);

        self->Maximize(SET_GUI_MAXIMIZED_CONSOLE->GetBool());
